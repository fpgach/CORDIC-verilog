...
int main() {
    ...
    for(int i = 0; i < (int)Fs; i++) {
        double  my_angle = i*f*360/Fs;
        int     my_modul = (1 << (AMP_MSB-1))-1;
        double  cor_angle = my_angle;
        while(cor_angle >= 360.0)
            cor_angle-= 360.0;

        cor_angle *= (1<<ARG_MSB)/360.0;
        int i_cor_angle = (int)cor_angle;
        //printf("%d new angle\n", i_cor_angle);
        calc_cordic(Re,Im, my_modul, i_cor_angle);
